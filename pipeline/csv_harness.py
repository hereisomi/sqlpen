...
def _extract_row_diagnostics(crud_result: CrudResult) -> List[Dict[str, Any]]:
    """Extract per-row failure details from CrudResult diagnostics."""
    diag = crud_result.diagnostics
    results = []

    # Bulk error
    bulk_err = diag.get("bulk_error")
    if bulk_err:
        cause, fix = _diagnose_error(str(bulk_err))
        results.append({
            "type": "bulk_failure",
            "error": bulk_err,
            "root_cause": cause,
    ...
    # Per-row errors from fallback
    for row_err in diag.get("row_errors", []):
        cause, fix = _diagnose_error(str(row_err.get("error", "")))
        results.append({
            "type": "row_failure",
            "row_index": row_err.get("row_index"),
            "error": row_err.get("error"),
            "root_cause": cause,
            "recommendation": fix,
        })

    # NOT NULL pre-check violations
    for nn in diag.get("not_null_violations", []):
        results.append({
            "type": "not_null_violation",
            "row_index": nn.get("row"),
            "column": nn.get("column"),
            "root_cause": "NOT NULL violation",
            "recommendation": f"Row {nn.get('row')} has NULL in NOT NULL column '{nn.get('column')}'. "
                              f"Fix: Provide a default value or drop this row.",
        })
    ...
    def summary(self) -> str:
        ...
        for s in self.steps:
            ...
            lines.append(
                ...
                f"| success={s.crud_result.success}  failed={s.crud_result.failed}  "
...
class PipelineRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # Build the test harness — this validates the (possibly prepared) data
        ...
        logger.info(
            "PipelineRunner initialized: table=%s, rows=%d, pk=%s, constraint=%s, "
            "pipeline=%s",
            table, len(prepared_df), pk_cols, constraint_cols,
            "full" if self.pipeline_applied else "raw",
        )
    ...
    def _prepare_data(
        ...
        # 1. Clean / Sanitize column names
        if self._clean:
            logger.info("[Harness] Sanitizing column names...")
            from utils.ddl import sanitize_dataframe_columns
            data, mapping = sanitize_dataframe_columns(
                data, server=dialect, allow_space=False, to_lower=True,
            )
            # Remap pk and constraint column names through the sanitizer
            pk_list = [mapping.get(c, c) for c in pk_list]
            con_list = [mapping.get(c, c) for c in con_list]
        ...
        if self._outlier and self._outlier > 0:
            ...
            data = replace_outliers_with_zero_safe(data, method='iqr', threshold=self._outlier)
    ...
    def _run_insert(self) -> StepResult:
        ...
        crud_result = auto_insert(
            self.engine, df, self.table, config=self.cfg,
        )
    ...
    def _run_upsert(self) -> StepResult:
        ...
        crud_result = auto_upsert(
            self.engine, df, self.table,
            constrain=self.harness.constraint,
            config=self.cfg,
        )
        elapsed = round(time.perf_counter() - t0, 3)
        ...
        if self.validate:
            step = self._validate_step(step, self.harness.get_upsert_check_query(),
    ...
    def _run_update(self) -> StepResult:
        ...
        if self.validate:
            step = self._validate_step(step, self.harness.get_update_check_query(),
    ...
    def _build_report_lines(self, report: PipelineReport) -> List[str]:
        ...
        for step in report.steps:
            ...
            lines.append(f"  Success:  {step.crud_result.success}")
            lines.append(f"  Failed:   {step.crud_result.failed}")
            lines.append(f"  Method:   {step.crud_result.method}")
            lines.append(f"  Elapsed:  {step.elapsed_s:.3f}s")

            # DML diagnostics
            diag = step.crud_result.diagnostics
            if diag.get("bulk_error"):
                lines.append("")
                lines.append("  DML ERROR (bulk attempt):")
                lines.append(f"    {diag['bulk_error']}")

            if diag.get("row_errors"):
                lines.append("")
                lines.append(f"  ROW-LEVEL ERRORS ({len(diag['row_errors'])} failures):")
                for re_item in diag["row_errors"][:20]:  # cap at 20
                    lines.append(f"    Row {re_item['row_index']}: {re_item['error']}")

            if diag.get("not_null_violations"):
                lines.append("")
                lines.append("  NOT NULL VIOLATIONS:")
                for nn in diag["not_null_violations"][:20]:
                    lines.append(f"    Row {nn['row']}, column '{nn['column']}'")
