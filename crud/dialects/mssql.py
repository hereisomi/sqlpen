"""MSSQL dialect — MERGE statement via raw SQL.

Does NOT depend on sqlalchemy.dialects.mssql.dml.Merge (which doesn't exist
in most SA versions). Uses portable raw SQL instead.
"""
...
class MssqlDialect(BaseDialect):
    ...
    def build_upsert_stmt(
        ...
        """Build MSSQL MERGE as raw SQL with sanitised bind parameters.
        ...
        merge = f"MERGE INTO {tbl_esc} AS tgt USING ({src_sql}) AS src ON {on_sql}"
        if update_sql:
    ...
    def _exec_upsert_bulk(
        ...
        """MSSQL MERGE — execute with remapped param keys."""
