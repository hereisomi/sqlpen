...
def escape_identifier(name: str, dialect: str = 'sqlite') -> str:
    """Escape an identifier (table or column name) based on the SQL dialect."""
    ...
    if dialect in ('mysql', 'mariadb'):
        ...
        # Keep original case; Oracle uppercases only when identifiers are *unquoted*.
        return '"' + name.replace('"', '""') + '"'
    else:
        # Default for PostgreSQL, SQLite, etc. (Postgres natively lowercases)
        return '"' + name.replace('"', '""') + '"'
...
def _build_condition(cd: Dict[str, Any], params: Dict[str, Any], dialect: str) -> Tuple[str, Dict[str, Any]]:
    ...
    if op == 'IN':
        if isinstance(val, str):
            val = _process_values('in', val) if val.startswith('(') else [val]
