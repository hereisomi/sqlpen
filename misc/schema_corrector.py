...
class SchemaAligner:
    ...
    def __init__(self, conn: Connection | Engine, db_type: str | None = None,
                 *, on_error: str = 'coerce', failure_threshold: float = 0.1,
    ...
    def align(self, df: pd.DataFrame, table: str, 
        ...
        aligned = self._coerce_types(df_mapped, meta, on_error, failure_threshold)

        self._enforce_nullability(aligned, meta, on_error, failure_threshold)
    ...
    def _coerce_types(self, df: pd.DataFrame, meta: Dict[str, ColumnInfo], 
                      on_error: str, threshold: float) -> pd.DataFrame:
        out = df.copy()
    ...
    # -------------------------------------------------------------------------
    def _enforce_nullability(self, df: pd.DataFrame, meta: Dict[str, ColumnInfo], on_error: str, threshold: float):
        for col_name, info in meta.items():
